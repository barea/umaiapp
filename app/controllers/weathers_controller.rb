...
class WeathersController < ApplicationController
  ...
  def export
     obj = OpenWeatherMap.new(params[:city_name], params[:dt])
    @weather = obj.fetch_data
    if @weather.present?
    @weatherHash = eval(@weather.weather_info)
    end
  end
  ...
  private
    ...
    def weather_params
      params.require(:weather).permit(:weather_info, :city_name, :country, :dt)
    end
end
